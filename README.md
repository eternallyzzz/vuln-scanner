...
- 补丁后复扫验证：任务成功后进入 `post_patch_status=pending`，下一次该 agent 的 CVE 匹配/复扫完成后按任务自身 `cve_ids` 判定——无 active 记为 `passed`，仍有 active 记为 `failed`（`post_patch_detail` 列出剩余 CVE），任务无 CVE 记为 `na`；超过 24 小时未复扫由 reaper 标记 `failed`，并追加 `patch_task.post_patch` outbox 事件
- 补丁闭包跟进：`failed` 且 `post_patch_detail` 含 missing fixes 时，自动为同一 agent/资产生成 follow-up campaign（`created_by=post-patch-followup`），新任务沿用 `patch.default_approval_required`；单链最多 3 层、每个源任务最多 3 次尝试，无 missing fixes 或已达上限时记录 `skipped`
